...
def convolution(image, kernel):
    # Ensure that the image and kernel are numpy arrays
    ...
    if is_color:
        # Separate color channels and apply convolution to each channel
        result = np.zeros_like(image)
        for channel in range(image.shape[2]):
            result[:, :, channel] = convolve2d(image[:, :, channel], kernel, mode='same', boundary='wrap')
    else:
...
im=skio.imread('images/lena.tif')
viewimage(rotation(im,45,clip=True,alpha=0.5,ech=1))
...
# %%
im=skio.imread('images/lena.tif')
noyau_gaussien = get_gau_ker(2)
viewimage(filtre_lineaire(im, noyau_gaussien))
# %%
im=skio.imread('images/lena.tif')
im= np.float32(im)
noyau_cte = get_cst_ker(2)
#viewimage(filtre_lineaire(im, noyau_cte))
viewimage(abs(filtre_lineaire(im, noyau_cte) - im))
# %%
im=skio.imread('images/pyramide.tif')
im= np.float32(im)
im_br = noise(im, 10)
noyau_gaussien = get_gau_ker(1)
noyau_cte = get_cst_ker(1)
viewimage(im_br)
viewimage(filtre_lineaire(im_br, noyau_cte))
# %%
...
noyau_gaussien = get_gau_ker(1)
noyau_cte = get_cst_ker(1)
#var_image(im, 0, 0, 1, 1)
viewimage(im_br)
viewimage(filtre_lineaire(im_br, noyau_gaussien))
viewimage(median_filter(im_br))
# %%
im=skio.imread('images/carre_orig.tif')
im= np.float32(im)
#viewimage(im)
viewimage(filtre_lineaire(im, [[0, 0, 1], [1, 1, 1], [0, 0, 1]]))
#im_br = noise(im, 10)
#noyau_gaussien = get_gau_ker(1)
#viewimage(filtre_lineaire(im_br, noyau_gaussien))
#im = filtre_lineaire(im_br, noyau_gaussien)
#viewimage(im_br)
# %%
im=skio.imread('images/carre_orig.tif')
im= np.float32(im)
viewimage(convolution(im, [[0, 0, 1], [1, 1, 1], [0, 0, 1]]))
# %%
im=skio.imread('images/carre_orig.tif')
im= np.float32(im)
im_br = noise(im, 10)
#viewimage(im_br)
viewimage(wiener(im, np.array([[0, 0, 1], [1, 1, 1], [0, 0, 1]]), 80))
# %%
im=skio.imread('images/carre_orig.tif')
im= np.float32(im)
#viewimage(im)
viewimage(wiener(noise(im,10),get_gau_ker(2),250))
# %%
im=skio.imread('images/carre_orig.tif')
im= np.float32(im)
im=noise(im,5)
...
for i in range(0,100):
    ...
    if(np.around(y,2)==np.around(x,2)):
        print(i)    
# %%
def new_wiener(input_image, kernel, regularization=0):
    ...
    extended_kernel = np.zeros((image_height, image_width))
    extended_kernel[:kernel_height, :kernel_width] = kernel

    half_width = image_width / 2
    half_height = image_height / 2

    frequency_x = np.concatenate((np.arange(0, half_width + 0.99), np.arange(-half_width + 1, -0.1)))
    frequency_y = np.concatenate((np.arange(0, half_height + 0.99), np.arange(-half_height + 1, -0.1)))

    frequency_x = np.ones((image_height, 1)) @ frequency_x.reshape((1, -1))
    frequency_y = frequency_y.reshape((-1, 1)) @ np.ones((1, image_width))

    frequency_x = frequency_x / image_width
    frequency_y = frequency_y / image_height

    w_squared = frequency_x**2 + frequency_y**2
    w = np.sqrt(w_squared)

    float_image = np.float32(input_image.copy())

    shifted_spectrum = np.fft.fftshift(abs(np.fft.fft2(float_image)))
    a = shifted_spectrum**2
    b = input_image.var() * input_image.size
    x = b / a

    g = np.fft.fft2(input_image)

    k = np.fft.fft2(extended_kernel)

    filter_multiplier = np.conj(k) / (abs(k)**2 + x)

    filtered_spectrum = g * filter_multiplier
    
    mm = np.zeros((image_height, image_width))
    y_shift = int(np.round(kernel_height / 2 - 0.5))
    x_shift = int(np.round(kernel_width / 2 - 0.5))
    mm[y_shift, x_shift] = 1
    output_image = np.real(np.fft.ifft2(filtered_spectrum * (np.fft.fft2(mm))))

    return output_image

# %%
im=skio.imread('images/carre_flou.tif')
viewimage(wiener(noise(im,10),get_gau_ker(2),17))
#viewimage(wiener_mod(noise(im,10),get_gau_ker(2),17))
# %%
from PIL import Image
input_image = Image.open("images/bat200.tif").convert("L")  # Replace with your grayscale image file

# Convert the PIL image to a NumPy array
input_image = np.array(input_image) / 255.0  # Normalize to [0, 1]

# Define a linear transformation function
def linear_filter(image, slope, intercept):
    filtered_image = image * slope + intercept
    filtered_image = np.clip(filtered_image, 0, 1)
    return filtered_image

# Apply linear filtering with desired slope and intercept
slope = 1.5  # Adjust as needed
intercept = -0.2  # Adjust as needed
output_image = linear_filter(input_image, slope, intercept)
viewimage(input_image)
...
#viewimage(im)
#viewimage(out)
plt.hist(out.reshape((-1,)),255)
plt.show()
print(im = out)
# %%
def rescale(img):
  return np.interp(img, (img.min(), img.max()), (0, +1))
input_image=skio.imread('images/facade.tif')
print(rescale(input_image)[0][0])

# %%
def Rc_combined(file_name, distance, r_function):
    Ic = skio.imread(file_name)
    Ic = Ic[:30, :30]
    rm = rmax('tanh')
    print(rm)
        ...
        for i in range(Ic.shape[0]):
            for j in range(Ic.shape[1]):
                    ...
                    for y in range(I.shape[1]):
                        try:
                            if x != a or y != b:
                                dx = abs(a - x)
                                dy = abs(b - y)
                                c += r_function(I[a][b] - I[x][y]) / d(dx, dy, distance)
                ...
                if s != 0:
                    ...
                    Rc[i][j][channel] = 0.0  # Set a default value when s is zero
    return Rc
...
Rc= Rc_combined('images/chat.tif', 'Euclidian', 'tanh')
plt.imshow(Rc)
# %%
