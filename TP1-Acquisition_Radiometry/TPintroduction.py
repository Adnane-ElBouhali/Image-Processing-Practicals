...
#%% charger une image 
im=skio.imread('images/chile.jpg')
...
print (r.shape)
#%% une image couleur 
im=skio.imread('images/after.jpg')
...
im=skio.imread('images/sombre.jpg')
im=im.mean(axis=2) #on est sur que l'image est grise
imequal=255*histocum[np.uint8(im)]
(histoeq,bins)=np.histogram(imequal.reshape((-1,)),np.arange(0,256)) #le reshape en inutile pour np.histogram, mais on le laisse pour la compatibilite avec plt.hist
histoeq=histoeq/histoeq.sum()
histocumeq=histoeq.cumsum()
viewimage(imequal)
plt.plot(histoeq)
...
plt.hist(abs(np.float32(unew)-np.float32(v)).reshape((-1,)),255)
...
image = io.imread('images/vue1.tif')
image_equalized = 255*exposure.equalize_hist(image)
viewimage(image_equalized)
(histo,bins)=np.histogram(image_equalized.reshape((-1,)),np.arange(0,256))
histo=histo/histo.sum()
histocum=histo.cumsum()
plt.plot(histocum)
# %%
from PIL import Image
input_image = Image.open("images/chile.jpg").convert("L")  # Replace with your grayscale image file
